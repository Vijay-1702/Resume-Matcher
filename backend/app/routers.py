...
def _ensure_suggestions_for_session(session_data: dict, db: Session):
    results = session_data.get("results") or {}
    if results.get("recommendations") or results.get("ai_suggestions") or results.get("aiSuggestions"):
        return results.get("recommendations") or results.get("ai_suggestions") or results.get("aiSuggestions")

    resume_version_id = session_data.get("resume_version_id")
    if not resume_version_id:
        return []

    resume_version = db.query(models.ResumeVersion).filter(models.ResumeVersion.id == resume_version_id).first()
    if not resume_version:
        return []

    jd = db.query(models.JobDescription).filter(models.JobDescription.id == resume_version.jd_id).first()
    match_result = db.query(models.MatchResult).filter(models.MatchResult.resume_version_id == resume_version_id).first()
    if not match_result:
        ...
        match_result.score = results.get("score", match_result.score)
        match_result.matched_skills = results.get("matchedSkills", match_result.matched_skills or [])
        match_result.missing_skills = results.get("missingSkills", match_result.missing_skills or [])
    ...
    results["aiSuggestions"] = suggestions
...
def workflow_upload_job_description_text(
    ...
    if session_data.get("resume_text"):
        ...
        _ensure_suggestions_for_session(session_data, db)
...
async def workflow_upload_resume(
    ...
    # Use session_id as user_id, default to 1
    user_id = int(session_id) if session_id and session_id.isdigit() else 1
    jd_id = 1  # default JD
